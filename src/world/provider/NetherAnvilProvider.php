...
class NetherAnvilProvider extends Anvil {
	...
	protected function pathToRegion(int $regionX, int $regionZ) : string{
		return Path::join($this->path, "dim-1", "region", "r.$regionX.$regionZ." . static::getRegionFileExtension());
	}

	public static function isValid(string $path) : bool{
		if(file_exists(Path::join($path, "level.dat")) and is_dir($regionPath = Path::join($path, "dim-1", "region"))){
	...
	private function createRegionIterator() : \RegexIterator{
		return new \RegexIterator(
			new \FilesystemIterator(
				Path::join($this->path, 'dim-1', 'region'),
